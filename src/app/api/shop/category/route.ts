...
import { CategorySchemaCreate } from "@/schemas/zod/shop";
...
/*
  ...
  Route: "api/shop/category" [ POST - GET ]
 
  GET: API handler for fetching all categories from the "ShopCategory" table in SurrealDB.
  POST: API handler for creating a new category in the "ShopCategory" table in SurrealDB.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const categoriesData = {
      parent_id: parent_id
        ? new RecordId(shopTables.category, parent_id)
        : undefined,
      name,
      slug,
      created_at: new Date(),
